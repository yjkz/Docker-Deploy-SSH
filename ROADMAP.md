...
> 用户裁决**跳过留档**(待密钥管理拍板后另行开工);L1 评估已完成(第三十四批(六):**值得做但建议单独立批**,
> 数据与结论见 UPGRADE-PLAN);**L1 的 0 期 spike 已完成(第三十六批(0 期) 2026-09-23,纯文档)** ——
> 唯一证伪点已打掉:**两种 image store 都接受裁剪包**(6/6 全绿,大镜像 233 MB 层用 2,243 B 增量装成);
> 附带发现「containerd 侧缺层时 `docker load` 仍 rc=0 且留坏镜像」与「镜像 ID 口径跨 store 不一致」(见 P5)。
> 1 期(单镜像最小闭环)是否立项待拍板。
...
| # | 项 | 落点与要点 | 规模 | 侦察依据(2026-09-22) |
|---|---|---|---|---|
| ~~L1~~ ✅ 评估 + **0 期 spike** 均已完成(第三十四批(六) v6.16.0 / **第三十六批(0 期) 2026-09-23**,纯文档) | **层级增量传输**(只传服务器缺失的层) | **0 期 spike 已打掉唯一证伪点**:受控自建镜像(`FROM scratch`,小 3 MB / 大 **233 MB** 基底层,v1→v2 仅改应用层一行)在两台真服务器上跑完矩阵 —— **正向 6/6 全绿**(containerd 产包 / 经典 store 产包 × {containerd, 经典} 两目标,裁剪后 load 全部 rc=0,内容级 md5 逐字节一致;大镜像全量 5.25 MB → **增量 2,243 B(0.043%)**,load **<1s**,内存波动仅 ±10~20 MB);**负向发现**:缺层时 **containerd 侧 `docker load` 仍 rc=0**,只在 **stdout** 打 `Error unpacking image … not found` 并**留下带标签的坏镜像**,而经典侧 rc=1(stderr `no such file or directory`)且无半成品 → **退出码不足以判成败**。**结论**:轻量裁剪路线**两种 store 都成立**,不需能力门控;实现要点 = 判据用 **diffID**、失败检测 rc+文本双判、重试前 `docker rmi` 清半成品、兜底重灌全量包(实测有效)。细节见 UPGRADE-PLAN 第三十六批(0 期);1 期(单镜像最小闭环)待用户拍板立项 | 大 | 工程 = 解析 save tar(两种 blob 命名路径)+ 远端 diffID 清单 + 流式重排;外部对照 unregistry/pussh(远起临时 registry 路线,已排除 —— 与「不依赖镜像仓库」定位冲突) |
...
#### 0 期 spike 顺带发现(2026-09-23 实测,独立记账,待复核)

| # | 项 | 实测与影响 | 规模 | 侦察依据 |
|---|---|---|---|---|
| P5(待复核) | **镜像 ID 口径跨 store 不一致** | 同一镜像在两台机器上 `.Id` 不同:**containerd store = 包内 `index.json` 的 manifest 摘要**(dd-l1big:v2 → `db0a685b…`),**经典 store = 包内 legacy `manifest.json` 的 Config 摘要**(同镜像 → `c926efd5…`);小镜像同样复现(`12196c7b` vs `140fd7e4`)。**影响面**:本地 Docker Desktop(4.34+ 默认 containerd store)↔ 经典 store 服务器之间,`same_image_id` 恒不成立 → **智能传输静默退化为全量上传**(纯性能损失,不误事);「按 ID 收敛」的回滚预检可能**误判「回不去」**而多要一次确认。**待复核**:无 attestation 的镜像是否同样;`docker images --no-trunc` 与 `.Id` 是否同源;是否影响既有真机案例(第三十批「标签被移走」场景) | 小-中 | L1 0 期 spike 实测(第三十六批(0 期);UPGRADE-PLAN 第七节) |
...
- 版本 **v6.18.0**(**已发版**,tag `v6.18.0` —— 含第三十七批 + 第三十八批 + 三轮复审修复);main = origin/main;基线 `cargo test` **552 passed** / 15 ignored（ignored 数此前长期写作 14;第三十四批(三) 加真机样板后实为 15,本批据实修正）
- 命令 **122** 个(generate_handler 实测;第三十三批 +8 文件管理 114→122;口径与 wiki/04 一致)
- **第三十八批 / v6.18.0(2026-09-23,与第三十七批同版本)**:L1 **整栈接入 + 归档契约 + 回滚第五态** —— ①**整栈逐包裁剪**:打包前查一次服务器已有层(建连留给步骤 3 复用),每个镜像独立走 `save_gzip_trimmed`(零增益即整包,与历史字节一致),产出裁剪包 + 兜底整包(本地 `<uuid>.tar.gz.full`;guard/断点/成功收尾同策略);断点产物增 `fullLocals` / `droppedLayers`,续传跳过打包时恢复裁剪信息。②**整栈装载循环**(单镜像同款):抓输出尾部 **rc+文本双判** → 命中即 `docker rmi -f` 清半成品 + 上传 `<包>.full` 整包重传再判(仍失败才报错);兜底远端名带 `.full` 独立后缀(同名会被 SFTP 大小比对续传拼坏)。③**归档契约(不变式)**:`releases/` 里的包默认必须是**完整归档** —— 裁剪包装载成功后由服务器 `docker save '<镜像>' -o 'T' && gzip -c 'T' > 'T.gz' && mv -f 'T.gz' '<归档名>' && rm -f 'T'` 重建(**刻意不用管道**:退出码取决于最后一条命令、`save` 失败 + `gzip` 收空输入会产出合法空包且 rc=0,pipefail 又依赖远端 shell);重建失败 → 删缺层裁剪包 + manifest `file=null` + 记 `needs_layers`(依赖层 diffID)。④**manifest** 增 `ManifestImage.needs_layers`(`serde(default)` + 空值不序列化,旧归档兼容);manifest 构建移到装载之后(归档结果步骤 4 才定)。⑤**回滚第五态「增量包待补层」**:`RollbackImageSource::NeedsLayers`(契约串 `needsLayers`)+ `plan_rollback_with_layers`(预检/执行链两入口共用;层清单按需查,无增量归档项时零额外往返)—— 缺任一依赖层 → **阻断并逐条列出**;全在 → 走既有 ID/标签判定且 detail 注明「增量归档(依赖服务器已有层)」;层清单不可得 → **不做**缺层断言(detail 注明未核对);前端两入口渲染第五态徽章与计数。⑥**词表统一**(部署日志/归档/回滚预检三处同一口径):`增量传输:跳过 N 层,省 X MB` / `整包传输` / `增量装载失败 → 整包重传:<原文>` / `完整归档(自包含,回滚可直接装载)` / `增量归档(依赖服务器已有层)`。⑦顺带修:`resume_local_tars` 补收单镜像 `full_local`(第三十七批「放弃断点」清理漏了兜底整包)。测试 543→**551**(+8),命令 122 不变;重建命令链已**真机冒烟**(tencent2 经典 store + kkhuawei containerd,自建 `FROM scratch` 受控镜像:成功链 rc=0 / `gzip -t` 通过 / 标准 OCI 布局,失败链 rc=1 且不破坏原归档文件;用后归零)。**复审补修(同日)**:装载失败判定统一为 `incremental::load_failure_reason`(rc+文本双判的单一入口)+ `exec_forwarded_tail_status` 软失败变体 —— **经典 store 侧缺层是 rc≠0**,此前硬失败会在到达双判前直接 Err,裁剪包走不到「清半成品 + 整包重传」;现两种 store 的失败形态都自愈(未裁剪路径行为与文案保持原样)。测试 551→**552**(+1)。**复审二(独立审查复核)**:无清单归档目录的回滚装载补同一 rc+文本双判(此前 containerd 侧缺层包会被 rc=0 蒙混过关、带着坏镜像继续 up);兜底包丢失与未裁剪路径的文案修正;`RollbackPrecheckItem.source` rustdoc 六态补全。**复审三(用户裁决「记录在案全部修正」)**:①`TempFileGuard` 语义改**先 armed 后 disarm** —— 断点产物登记前的 tar 失败即删(此前断点模式下打包失败会留孤儿),登记后 disarm 交给断点/成功收尾;②兜底重传前的 `docker rmi` 先按**本次期望镜像 ID** 核对(此前会误删上一版标签指向的镜像,让更早归档落 Missing);③SSH `keepalive_interval = 30s`(长打包期间复用连接不再被空闲超时掐断)。**已发版(v6.18.0)**
- **第三十七批 / v6.18.0(2026-09-23)**:**层级增量传输(L1)落地** —— 按用户裁决「整个 L1 做完」实施。①**纯函数内核** `src-tauri/src/incremental.rs`:解析 `docker save` 的 OCI 混合布局,按**下标**把 `manifest.layers[]` 的 blob 名与 `config.rootfs.diff_ids[]` 配对(两种包来源都成立:containerd 产包 blob 名是压缩摘要、经典 store 产包 blob 名就是 diffID);`plan_drop` 按远端 diffID 集合求可丢层;`write_trimmed_tar` **流式**重打包(层可能 GB 级,绝不整块进内存);`detect_silent_load_failure` 做 rc+文本双判。**9 个单测**含 0 期实测原文 golden(containerd 的 `Error unpacking image …`、经典 store 的 `no such file or directory`)+ 合成 OCI 包 fixture 的解析/裁剪/字节一致性断言。②**导出侧** `docker::save_gzip_trimmed`:**零增益即零改动** —— 先比对本地层与远端清单,无交集直接返回 None 走原 `save_gzip`(与历史字节一致、不落裸 tar);有交集才落裸 tar、裁剪、两趟 gzip(裁剪包 + 兜底整包)。③**部署侧(单镜像)**:预查询复用连接;`server_deploy` 装载改 `exec_forwarded_tail` 抓尾部 → 命中失败标志即 `docker rmi` 清半成品 + 上传 `<tar_name>.full` 整包重传(远端名必须不同 —— 同名会被 SFTP 远端大小比对续传把整包续在裁剪包后面而损坏);断点产物 `full_local` + 成功收尾同策略清理。④新增 `tar = "0.4"` 依赖。⑤设置项 `AppSettings.incremental_transfer`(默认 true)+ 设置中心「部署」区开关。**未做**:整栈管线接入(含服务端重建归档)留待下批,整栈行为与历史一致。测试 534→**543**(+9),命令 122 不变
- **第三十六批(0 期)/ 无版本变更(2026-09-23,纯文档)**:L1 层级增量传输**可行性 spike** —— 按用户裁决先打掉唯一证伪点(**零生产代码、零命令、零测试变更,故不 bump**)。**授权边界**:用户指定**仅 tencent2 与 kkhuawei** 两台、**禁用服务器现有镜像**(自建受控镜像)、**注意内存限制**。①**环境矩阵**(只读探查):本地 Docker Desktop 29.7.2 = containerd store;tencent2(OpenCloudOS 9.6 / Docker 28.0.1)= **经典 store**(`overlay2`);kkhuawei(Ubuntu 24.04 / Docker 29.8.1)= **containerd store**(Linux 真机)。**特征检测判据当场产出**:`docker info --format '{{json .DriverStatus}}'` 含 `driver-type=io.containerd.snapshotter.v1` 即 containerd。②**受控镜像全部自建**(`FROM scratch`,零拉取):小镜像 3 MB 基底层 + 130 B 应用层;大镜像 **233 MB** 基底层;v1→v2 一律只改应用层一行(基底层 diffID 逐字节一致,精确复刻「重建但层重叠」)。③**包布局两种**(实测):containerd 产 = OCI + 兼容 manifest.json、层 blob 名 = **压缩摘要**;经典 store 产 = 同布局 + `repositories`、层 blob 名 = **diffID 且层未压缩**。④**正向矩阵 6/6 全绿**:两种来源的裁剪包 × 两种目标 store,`docker load` 全部 rc=0 且**内容级 md5 逐字节一致**(小镜像 3 MB 层、大镜像 **233 MB** 层各验一遍);大镜像全量 gzip 5,251,181 B → **增量包 2,243 B(0.043%)**,load **<1s**,内存波动仅 ±10~20 MB。⑤**负向矩阵(本轮最关键)**:缺层时 **containerd 侧 `docker load` 仍 rc=0**,只在 **stdout** 打 `Error unpacking image … content digest … not found` 且**留下带标签的坏镜像**;经典侧 rc=1(stderr `no such file or directory`)且无半成品 → **退出码不足以判成败**。⑥**对 1 期的直接结论**:不需按 store 做能力门控(都能用);判据必须用 **diffID**(store 无关);失败检测 rc + 文本双判、重试前 `docker rmi` 清半成品;兜底 = 重灌全量包(**实测有效**,共享 blob 还会自愈旧坏镜像)。⑦**顺带发现(P5,独立记账待复核)**:镜像 ID 口径跨 store 不一致 —— containerd 侧 `.Id` = manifest 摘要(`db0a685b…`),经典侧 = Config 摘要(同镜像 `c926efd5…`),致本地 containerd ↔ 经典服务器之间智能传输静默退化为全量、按 ID 收敛的回滚预检可能误判「回不去」。⑧**归零**:两台服务器测试镜像/容器/`/tmp` 全清,镜像与容器计数回到基线(19/3、12/1)、内存回基线,全程未碰现有镜像/容器/卷/网络/compose 项目/daemon 配置,**未执行任何 prune**。细节见 UPGRADE-PLAN 第三十六批(0 期)
- **第三十五批 / v6.17.0(2026-09-23;已发版)**:**设置中心信息架构重构 + 应用图标品牌化**(用户驱动:「设置太臃肿,要滑半天到底」+「需要换个软件图标」) —— ①**设置模态改「左分类导航 + 单区显示」**:原「通用」组独吞 17 项(19 控件 + 19 段 hint)、卡片无 max-height、滚动落在遮罩层(标题栏随内容滚走),实测约 3 屏;现拆 **外观 / 通用 / 部署 / 监控 / 更新 / 关于** 六分区,每区 ≤6 项。**实现要点**:分区 DOM 一次性全建、`selectSection` 只切 `.hidden` —— 保住 `fillSettings` / `onSave` 的 `getElementById` 契约与「切区不丢未保存输入」(桩验证实测:在「更新」区改动代理值 → 切到「外观」「关于」再回来,值仍在);卡片定高 `min(88vh, 100vh−96px)` + 纵向弹性正文(定高口径抄 `.help-modal-card`,**取 88vh 而非 80vh** 是因为「部署 / 监控」区 5-6 项带长 hint 刚好压在 80vh 红线上,实测提升后部署区一屏放下),**只有 `.settings-content` 滚动**,「保存设置」移出滚动区钉在卡片底部;导航项与 `.help-nav-item` 共用同一条样式规则(左缘信号青 + `--ark-signal-dim` 底),英文微标签复用 `.form-label-en`;全站 id 一个未改,`static-integrity` 的 id 三查不受影响。②**两处口径修正**:「检查更新 / 测试连接」两钮从底部动作行移入「更新」区(只作用于该区代理框);**读取设置失败**由「结果区行内回显」改走常驻顶部 `#settings-error` —— 结果区在「更新」分区内,原写法会在默认显示的「外观」区里静默;保存成功不再写行内回显(同样看不见,且事后重进该区会成为过期文案),反馈统一由 toast 承担。③**应用图标品牌化**:此前全仓从未做过图标(脚手架默认 Tauri 黄 `#FFC131` / 青 `#24C8DB`,与本项目信号青 `#18d1ff` 不一致,16px 已糊且全仓无任何生成脚本);母题经多轮 PIL 渲染对比定稿(候选:箭头压在方框顶边 / 箭头整个浮起 / 全青单色 / 底缘青条 / 箭尾止于顶边 —— 后者小尺寸读成「金字塔」出局;**定稿「箭头浮起 + 粗箭杆」:箭头头部悬在方框顶边上方 72px、箭杆 112 宽跨过间隙**,32 / 24px 下青色质量最足),几何 = 墨底直角满底 + 纸白方框(576×480,描边 84)+ 信号青上行箭头;新增 `src-tauri/icons/source/generate_icon.py`(同一几何同时出 `icon.svg` + 1024 PNG,超采样抗锯齿,入库可复现),`npx tauri icon` 重生成全套(ico 六帧 / icns / 32·128·128@2x / Square* / StoreLogo;顺带清掉本项目用不到的 android / ios / 64x64 产物);`tauri.conf.json` 补 `bundle.windows.nsis.installerIcon` + `uninstallerIcon`(**未打真机安装包实测**,仅按 schema 与上游模板配置,已如实标注)。④**验证**:`node --check` + `verify/` 五脚本(static-integrity / user-facing-copy / bridge-integrity / scope-integrity / contract-smoke)全绿;Tauri 桩 + 本机静态服务(脚本与 style.css 带 `?v=` 破 IAB 缓存)截六分区 + 暗色 + 窄窗(≤760px 导航降级横向可滚条)共 8 图自查;`cargo test` 534 passed **亲眼确认**(零 Rust 改动,clippy 11 条告警与基线一致)。⑤**文档同步**:wiki/03 设置章节整节重写(含分区归属)、**help.js 三处「设置中心『通用』区」措辞改区名**(部署日报 → 监控;宿主机可写目录 → 部署 —— 帮助是历次收尾的盲区,本批显式核查)、wiki/README + 七篇页首戳 + 三处版本号 → v6.17.0。测试 534 不变,命令 122 不变
- **第三十四批(五)/ v6.16.0(2026-09-22,同一未发版版本)**:宿主机**白名单读写**(第三十三批留档清尾;用户裁决「白名单目录可读写」) —— ①设置项 `AppSettings.hostWritePaths`(绝对路径前缀,默认空 = 维持只读;归一化去空白/尾斜杠、拒 `..` 与相对路径,cap 10);②**判定在服务端**:入口闸门把目标路径与白名单条目一次性 `readlink -f` canonicalize 后按目录边界匹配(白名单取「字面 ∪ canonical」,软链不能逃逸),不通过即拒并指向设置中心;③只放开写、不收紧读;仅「部署目录」源(容器/卷不受限);覆盖前备份与二次确认沿用;④**审计顺带修掉两个真缺口**:`upload_local_file` 对 DeployDir 的写入分支是 `unreachable!()`(白名单放开后会 panic)、备份分支恒「不存在」(会**静默跳过备份**),均按宿主机 `cp` 语义补齐;⑤前端词法 UI 门控(设置新字段 + 文件管理两态提示/按钮),真正判定恒在后端。测试 531→**534**,命令 122 不变
- **第三十四批(四)/ v6.16.0(2026-09-22,同一未发版版本)**:容器内**多选批量传输**(第三十三批留档清尾;用户裁决「做」) —— ①列表行首常显勾选列 + 表头全选(半选态),勾选就地更新「已选 N 项 / 批量下载 / 清除选择」批量条(不整块重绘);②**批量下载**(文件 + 目录,目录服务端自动打包;选一次本机目录,逐条串行)与**批量上传**(`dialog.open({multiple:true})` 一次选多个本地文件,复用单条备份语义);③进度复用 `files-transfer-progress`(opLabel 带 (i/N)),**取消即中止**并标记剩余项「已取消(未执行)」,结束渲染逐条结果(三列对齐)+ toast 汇总;④纯前端(零新命令);**桩验证顺带抓到「预览页 style.css 未破缓存 → 版面未生效」的验证方法学问题**,已修正生成器并同步 AGENTS.md 的 IAB 缓存提示;judge 两图 pass(node --check / static-integrity 首跑抓出 `files-batch-count` 缺 id 悬空引用 → 已修)
...
- **候选池(2026-09-22 入库,第三十一批预备)**:~~P1 孤儿容器~~ / ~~P2 `--pull never`~~ / ~~S1~~ / ~~S2~~ **已随 v6.13.0 完成**;~~S4~~ / ~~S5~~ **已随第三十四批(一) v6.15.0 完成**;~~P3~~ **已随第三十四批(二) v6.16.0 完成**;~~S3~~ / ~~L2~~ **已随第三十四批(三) v6.16.0 完成**;P4 用户裁决**跳过留档**;~~L1~~ **评估 + 0 期 spike 均已完成(第三十四批(六) / 第三十六批(0 期);结论 = 两种 image store 都成立,1 期待拍板)**。详见「待完成 → 候选池(2026-09-22 入库)」
- **第三十四批(六)/ v6.16.0(2026-09-22,同一未发版版本;纯文档)**:层级增量传输**评估**(L1) —— 受控实测(本机 Docker 29.7.2 / containerd 存储):同构两版镜像(仅改应用一行)整包均为 3,651,072 B,而 v2 独有层合计仅 **1,214 B**;**把 v2 save 包只删掉服务器已有 2 个层 blob 后重打包(30,720 B)→ `docker load` 成功且镜像可运行** → 「按远端已有层裁剪」轻量路线实测可行(无需临时 registry);外部对照 unregistry/pussh 的「远起 registry + SSH 隧道 push」为成熟替代。**结论**:值得做但**单独立批**(归档一致性——同一 tar 兼传输/归档/回滚三职、OCI/legacy 双格式与 store 兼容、分片续传为主要风险,须全量兜底);是否立实现批待用户拍板。实验后本机 Docker 已归零(9 个遗留卷未触碰)
