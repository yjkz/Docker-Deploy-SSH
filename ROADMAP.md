...
| # | 项 | 落点与要点 | 规模 | 侦察依据(2026-09-22) |
|---|---|---|---|---|
| ~~L1~~ ✅ 评估已完成(第三十四批(六) v6.16.0,纯文档) | **层级增量传输**(只传服务器缺失的层) | 实测(本机 Docker 29.7.2,受控 alpine 模型):整包 3.65 MB vs **裁剪包 30 KB**;裁剪后 `docker load` **成功且可运行**(仅删服务器已有层 blob)→ 「按远端已有层裁剪」轻量路线可行。**结论**:值得做但**单独立批**(归档一致性/格式兼容/分片续传为主要风险,须全量兜底);是否立批待用户拍板 | 大 | 工程 = 解析 save tar + 远端层清单 + 流式重排;外部对照 unregistry/pussh(远起临时 registry 路线) |
...
- **候选池(2026-09-22 入库,第三十一批预备)**:~~P1 孤儿容器~~ / ~~P2 `--pull never`~~ / ~~S1~~ / ~~S2~~ **已随 v6.13.0 完成**;~~S4~~ / ~~S5~~ **已随第三十四批(一) v6.15.0 完成**;~~P3~~ **已随第三十四批(二) v6.16.0 完成**;~~S3~~ / ~~L2~~ **已随第三十四批(三) v6.16.0 完成**;P4 用户裁决**跳过留档**;~~L1~~ **评估已完成(第三十四批(六);结论 = 值得做、建议单独立批,待拍板)**。详见「待完成 → 候选池(2026-09-22 入库)」
