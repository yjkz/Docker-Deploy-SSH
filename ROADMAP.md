...
| # | 项 | 落点与要点 | 规模 | 侦察依据(2026-09-18) |
|---|---|---|---|---|
| C1 | **传输中可取消** | 给 `sftp_upload`(`ssh.rs:313-320`)/`save_gzip_remote`(`migrate.rs:389-453`)加块级取消检查,让大镜像/大卷传输中能即时中断 | 中 | 现状:取消只在镜像/卷边界生效(`migrate.rs` 检查点 268/322/347)。**可插点已有**(`copy_file_to_remote` 的 64KB 块循环 `ssh.rs:668-682`、保存流的 `channel.wait()` 循环),但**签名变更波及 14 个调用点**(deploy 6 / migrate_project 5 / migrate 1 / rollback 1 / ssh 测试 2)→ 属接口级扩展,机械改动多但语义简单 |
