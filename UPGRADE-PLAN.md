...
# 第三十四批(六):层级增量传输评估(L1,2026-09-22)

> **来源**:第三十四批方案(L1 为**评估项**:先量数据,再定规模);**版本**:v6.16.0(同一未发版版本;
> **纯文档交付,零生产代码**)。评估完成 → 是否立实现批由用户拍板。

## 实验一(本机 Docker 29.7.2,containerd 镜像存储)

**受控模型**:alpine 3.20 基础 + 3 MB「稳定依赖/资源」层 + 小应用层;两次构建**仅改应用文件一行**
(基础镜像经镜像加速源 `docker.m.daocloud.io` 拉取 —— 本机 Docker Desktop 未配 HTTPS 代理,Docker Hub 直连超时)。

| 量 | v1 | v2 | 说明 |
|---|---|---|---|
| `docker save` 输出 | 3,651,072 B | 3,651,072 B | 整包(**现状传输口径**) |
| gzip 后(≈现状实际传输量) | 3,625,768 B | 3,625,352 B | v1→v2 仅差 416 B |
| 层清单(压缩 blob) | 4 层 | 4 层 | 共享 2 个(基础层 3,630,321 B + 稳定层 3,118 B);v2 独有 2 个,**合计 1,214 B** |
| **裁剪包(只留缺失层)** | — | **30,720 B** | 丢弃 2 个共享层 blob 后重打(OCI 布局:index.json → image index → image manifest → config+layers) |

**可行性实测(关键)**:在 v1 已导入的前提下,把 v2 的 save 包**只删掉服务器已有的 2 个层 blob**,
`docker load` **成功**(`Loaded image: dd-l1-real:v2`),导入后镜像**可正常运行**(`cat /app.txt` → v2 内容)。
即:**「按远端已有层裁剪 save 包 → `docker load`」这条轻量路线在实测环境可行,无需起临时 registry。**

**外部实践对照**:unregistry / `docker pussh`(2025 HN 热帖)明确写道「`docker save | ssh docker load` 即使
服务器已有 90% 内容也会整包传输」,其实现走**远端起临时 registry + SSH 隧道内 `docker push`**(去重由
registry 协议承担,不赌 `load` 容错)。

## 结论与建议(待用户拍板)

- **收益**:在「基础/依赖层稳定、仅应用层变化」的常见形态下可跳过 **>99%** 传输量(本实验 1.2 KB vs 3.65 MB;
  真实镜像按 unregistry 口径亦 ≥90%)—— 对慢上行链路收益显著。
- **可行**:轻量路线已实测可用;重路线(临时 registry)是成熟实现的选择,代价是远端起一个容器。
- **工程量与风险(评估)**:
  1. 需拆 `docker save` 两种布局(OCI / legacy)并做「远端已有层(diffID 集合,`RootFS.Layers`)↔ 本包 blob
     摘要」映射(经 config 的 `rootfs.diff_ids` 对齐),再做流式重排打包;
  2. **归档一致性是主要风险**:现有同一条 tar 同时承担「传输 / 归档到 release / 回滚装载」三职;传输裁剪后,
     归档与回滚必须仍能拿到完整链 —— 可行方案:归档仍写完整包(磁盘代价不变),或回滚侧改为「按 manifest +
     远端层清单校验,缺层即要求重传」(牵动回滚预检与断点续传);
  3. 兼容性:containerd 与经典 image store、Docker 版本差异都可能让裁剪包 `load` 失败 → 必须**全量兜底**
     (任何不确定即走现状整包),且兜底需真机矩阵验证;
  4. 传输/取消/进度/断点续传体系要与「分片包」形态对齐(分片要么重传、要么记录每片完成位)。
- **建议**:「值得做,但**单独立批**实现」(规模:大)—— 最小闭环 = 「基础层稳定」场景 + 全量兜底 + 真机矩阵;
  若不优先,可维持现状整包传输,本评估数据留档备查。

## 验证与现场

- 实验全部在本机 Docker 完成;**实验后已归零**(镜像 0 / 容器 0 / Build Cache 已清;9 个遗留卷未触碰),lab 目录已删
- 零生产代码改动 → `cargo test` 534 不变;verify 七脚本 rc=0(文档改动仅 ROADMAP/README/本文件)

---

# 第三十五批:设置中心六分区 + 应用图标品牌化(2026-09-23)

> **来源**:用户直提两条 —— 「现有的设置中太过臃肿 要滑半天才能到底下 对此进行分类优化」
> 「我需要换个软件图标」;**版本**:v6.17.0(v6.16.0 已打 tag 发版,本批独立 bump)。
> **纯前端 + 资源**,零 Rust 改动、零新命令(命令 122 / 测试 534 均不变)。

## 一、设置中心:左分类导航 + 单区显示

**问题(实测)**:设置不是页面而是独立模态 `#settings-modal`(每次打开 `buildBody` 重建),
19 个控件 + 19 段 hint **全塞在单个「通用」分组**里;`.settings-modal-card` 只设宽度、无 max-height,
滚动落在遮罩层 `.modal-overlay` 上 —— 标题栏跟着内容一起滚走,1080p 下约 3 屏。

**分区**(`SECTIONS` 数组,顺序即导航顺序):

| 分区 | 内容(现有 id 一个未改) |
|---|---|
| 外观 | 主题三选(即时生效,唯一不经后端的分区) |
| 通用 | 关闭到托盘 / 启动从源更新 / 启动静默检查更新 / 开机自启 / 终端日志保留 / 打开日志文件夹 |
| 部署 | 失败自动回滚 / compose 文件名 / 扫描最大深度 / 卷搬运 tar 镜像 / 宿主机可写目录 |
| 监控 | 探活间隔 / 告警采样间隔 / 磁盘·内存·CPU 阈值 / 部署日报时刻 |
| 更新 | 代理地址 / 检查更新 / 测试连接 / 结果区 |
| 关于 | 头像 / 名字 / 三条外链(原右侧常驻栏) |

**关键决策**:
1. **分区 DOM 一次性全建、只切 `.hidden`**(而非按需重建):`fillSettings` / `onSave` 是按
   `getElementById` 全量取值的,保留 DOM ⇒ 保存逻辑零改动,且来回切区不丢未保存输入。
   代码上用「分区游标 `main`」实现:各字段块按归属改指 `main = sections.<key>`,**各分区内部保持原有先后顺序**
   (逐个把原 `appendChild` 流劈成四条支流,不做大段搬移)。
2. **定高卡片**:`min(88vh, 100vh − 96px)` + flex 纵向,口径抄 `.help-modal-card`(80vh)。
   取 88vh 的理由:**部署 / 监控区 5-6 项带长 hint 刚好压在 80vh 红线上**(桩实测:80vh 下两区仍需内滚,
   88vh 下部署区一屏放下、监控区仅余最后一行 hint);`100vh − 96px` 保证不溢出遮罩的 48px 上下留白。
3. **导航项样式与 `.help-nav-item` 合并到同一条规则**(加一个选择器即可,零风险):
   左缘 3px 信号青 + `--ark-signal-dim` 底是站内既有的「当前项」语言,不新造组件语言;
   英文微标签直接复用 `.form-label-en`(cond 轨道 + 大写 + .14em)。
4. **两处口径修正**(原写法在新布局下会静默):
   - 「检查更新 / 测试连接」移入「更新」区 —— 它们只作用于该区的代理输入框;
   - **读取设置失败改走常驻顶部 `#settings-error`**(`formFailLoud`):结果区在「更新」分区内,
     用它报错会在默认显示的「外观」区里看不见;保存成功也不再写行内回显(同样看不见,事后重进该区
     还会变成过期文案),成功反馈统一由 toast 承担。

**改动清单**:

| 文件 | 内容 |
|---|---|
| `ui/settings.js` | `SECTIONS` + `selectSection` + `buildBody` 重写(分区游标分流)+ 「关于」改分区 + 两处回显口径 |
| `ui/style.css` | `.settings-modal-card` 定高 / `#settings-modal-body` 弹性列 / `.settings-nav`·`.settings-content`·`.settings-section` / `.settings-actions` 常驻 / 窄窗横向导航;`.help-nav-item` 规则并入 `.settings-nav-item`;`.about-card` 限宽 360px |
| `ui/help.js` | 三处「设置中心『通用』区」改区名(部署日报 → 监控;宿主机可写目录 → 部署) |
| `wiki/03-前端说明.md` | 设置章节整节重写(六分区 + 归属 + 两处口径修正) |

## 二、应用图标品牌化

**问题**:全仓从未做过图标 —— 现资产是脚手架默认的 **Tauri 官方 logo**(黄 `#FFC131` + 青 `#24C8DB`
双弧钩 + 圆点),与本项目信号青 `#18d1ff` 不一致,16px 下已糊;全仓无任何生成脚本(`git log` 里
`src-tauri/icons/` 只出现在脚手架提交)。

**定稿过程**(PIL 程序化渲染多轮对比,每轮都看 256 / 128 / 64 / 48 / 32 / 24 / 16px 与亮暗底):

1. 首轮四变体(白框+青箭 / 全青单色 / 底缘青条 / 标准笔画)—— 发现**箭杆占方框顶边 21%,把顶边切成两截**,
   小尺寸读成「桌子」;全青在 16px 读成 house 轮廓。
2. 次轮改几何(箭杆收窄、方框缩小)+ 加 **16px ×8 最近邻放大检视行** —— 这一步是本批方法学关键:
   不经放大根本看不出小尺寸到底糊成什么。
3. 三轮扫「箭头头部相对顶边的高度」轴:压住顶边 / 紧贴上沿 / 浮起 72px / 浮起且箭杆更粗 ——
   压住顶边者在 16px 会与方框糊成一体。
4. 终选:**「箭头浮起 + 粗箭杆」** —— 头部悬在顶边上方 72px、箭杆 112 宽跨过间隙;
   「箭尾止于顶边」变体在 16px 读成「金字塔/屋顶」而出局。
5. 产出:**墨底 `#080a0b` 直角满出血 + 纸白 `#f4f6f6` 容器方框(576×480,描边 84)+ 信号青上行箭头**
   (母题 = 把本地镜像送出去:方框是容器,箭头突破边界向上)。

**交付物**:
- `src-tauri/icons/source/generate_icon.py` —— **可复现生成器**(Python + Pillow;4× 超采样抗锯齿),
  同一份几何同时输出 `icon.svg`(矢量留档)与 `icon-1024.png`(栅格源);改形状只需改常量。
- `npx tauri icon src-tauri/icons/source/icon-1024.png` 重生成全套:ico(16/24/32/48/64/256 六帧)/
  icns / 32·128·128@2x / Square* / StoreLogo;顺带产出的移动端(`android/`·`ios/`)与 `64x64.png`
  本项目用不到,已清并写进生成器文档。
- `tauri.conf.json` 补 `bundle.windows.nsis.installerIcon` + `uninstallerIcon`(两者此前缺失,
  安装/卸载向导走 NSIS 默认图标)。**未打真机安装包实测** —— 仅按本地 schema(`NsisConfig.installerIcon`
  存在且有同族 `uninstallerIcon`)与上游模板(`mod.rs` 仅在 `Some` 时注入 `MUI_ICON`)配置,如实标注。

## 三、验证与现场

- `node --check ui/settings.js` + `verify/` 五脚本全绿(static-integrity 的 id 三查 / user-facing-copy 的
  文案断言均未受影响 —— 后者是「hint 文案一字不改、只挪 DOM」的直接原因)
- Tauri 桩 + 本机静态服务(脚本与 style.css 均带 `?v=` 破 IAB 缓存)截 **8 图**:六分区 + 暗色主题 +
  窄窗(≤760px 横向导航)自查;另用页面内脚本验证三件事:**23 个 id 齐全** / **切区不丢未保存输入** /
  保存链路无异常且 `window.__stubErrors` 为空。脚手架(桩 + 预览页 + 静态服务 + 页面)已全部清退
- `cargo test` **534 passed / 0 failed**(本批零 Rust 改动,仅确认基线);`cargo clippy` 11 条告警与基线一致
...
## 追加:复审修复(用户复审要求,同日;计数 551→552 见下)

复审「着重审查回滚与部署」发现一处**部署侧自愈缺口**并修复:

- **经典 store 侧缺层不自愈**:0 期实测两种 store 的失败形态相反(containerd rc=0 静默、
  经典 store rc≠0 显式)。原实现里装载用 `exec_forwarded_tail`,它的**硬失败语义**在
  rc≠0 时直接 `Err` —— 于是经典 store 的裁剪包装载失败**永远走不到**「清半成品 +
  整包兜底重传」(只有 containerd 的 rc=0 静默失败能走到),与交付标准「装载失败能自愈」
  不符。**修复**:①`incremental::load_failure_reason(exit_code, output_tail)` 作为
  rc+文本双判的**唯一入口**(先扫静默标志,再按 rc 兜底;原因取输出最后一条非空行,
  空输出给退出码文案);②`commands` 新增 `exec_forwarded_tail_status`(软失败变体:
  rc≠0 不转 Err,返回 `(退出码, 尾部行)`);③单镜像 5.1 与整栈步骤 4 的装载改用
  软失败变体 + 统一判定 —— 有兜底(裁剪过)→ 清半成品 + 整包重传;**未裁剪路径保持
  原样**(rc≠0 仍按 `远端命令执行失败(退出码 N): <cmd>` + 尾部全文报错,避免
  「未裁剪却说增量装载失败」的误导文案)。测试 551→**552**(+1)。
- 复审确认项(无改动):**自动回滚**走 `rollback_execute_stack_inner`,与 04 页共用同一段
  预检 → 第五态 + 层核对**已覆盖自动路径**;回滚链装载只装 manifest 记录的包,第五态
  服务 `file=null` → 用户确认后沿用服务器当前镜像(与文案一致);迁移链搬归档包
  (不变式下都是完整包)不受影响。
- 复审记录在案的**已知取舍/风险**(不改,供后续批次):
  ①增量路径临时文件峰值 ≈ raw + trimmed + 两 gz(约 3~4 倍压缩层总量),而导出盘预检
  仍是 `Σ镜像 × 1.5` —— 大镜像 + 低压缩率内容 + 并发 3 时预检可能偏紧(失败形态是
  打包报错,不损坏数据);
  ②`needs_layers` 的层清单查询失败时**不做**缺层断言(detail 注明「未能核对」,与
  「查不到镜像列表 ≠ 镜像丢了」同款留有余地)—— 保守派可在后续批次改为 fail-closed。
- 验证:`cargo clippy --lib` 11 条与基线一致(修复过程中新引入的一条
  `filter(..).next_back()` 告警已按建议改 `rfind`,零残留)。

### 追加(二):独立审查复核(同日)

派独立审查者对 `73f8817..HEAD` 全文复核(只读),结论「未发现数据损坏 / 错误版本回滚 /
静默跳过回退级问题」;报出 1 处**静默面** + 2 处低级问题,均在本轮修掉:

- **无清单归档目录的回滚装载未做双判**(静默面):失败/中断的部署可能在
  `releases/<ts>/` 留下**缺层裁剪包且无 manifest**(manifest 在健康检查通过后才写),
  用户从 04/06 页选这类目录回滚时装载按「目录内全部包」走 —— containerd 侧缺层包
  rc=0 会静默报「装载成功」并留下带标签的坏镜像,回滚继续 up。**修复**:两处回滚装载
  (`rollback.rs` 04 页与 06 页执行链)改用 `exec_forwarded_tail_status` +
  `incremental::load_failure_reason`(与部署链**同一收口**),命中即中止并保留既有
  「已装载 i/n,容器未重建」的部分失败文案。
- **未裁剪路径 rc=0 的文案误导**:原「增量装载失败且本地没有整包可重传(可关闭
  「层级增量传输」后重试)」在未启用增量时给出无从执行的指引 —— 改为「装载失败且本地
  没有整包可自动重传:<原因>(如本次启用了「层级增量传输」,可关闭后重试)」。
- **兜底包已丢失时的错误信息错位**:续传时本地 `<uuid>.tar.gz.full` 若已被清走,
  报的是上传阶段的「读取本地文件元数据失败」—— 重传前加存在性检查,给「本地兜底
  整包已丢失,请放弃该断点后重新部署:<原因>」。
- 顺带:`RollbackPrecheckItem.source` 的 rustdoc 六态补全(此前仍写四态)。
- 审查方记录在案、**本轮不改**的既有项(供后续批次评估):①打包失败(断点模式)时本地
  tar 成孤儿(既不进断点、也不进成功清理表;本批新增的 `<uuid>.tar.gz.full` 使单次
  泄漏量翻倍);②兜底重传前的 `docker rmi -f <tag>` 在「装载失败但标签未移动」形态下
  会删掉该标签原指向的上一版镜像(更早归档可能落 `Missing` —— 预检会显式列出,
