    ...
    },
    {
      ...
      html: `
        ...
        <ul>
          ...
          <li><strong>批量部署</strong>(页首「批量部署」按钮):勾选多台服务器后按队列<strong>串行</strong>执行当前模式(单镜像 / 整栈)的完整部署,每台独立环境预检、独立写历史(可逐台回滚);面板实时显示每台状态。</li>
          <li>点「停止批量」后<strong>当前这台会先跑完</strong>,余下标记「已跳过」(批量不做断点记录,中途打断会留下无法续传的半成品);若需立即中止当前台,请用「取消部署」——它会把当前台记为已跳过并同样停掉余下的服务器。</li>
        </ul>
        <h4>项目跨服务器迁移</h4>
        <ul>
