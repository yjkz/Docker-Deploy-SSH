      ...
      html: `
        ...
        <table>
          ...
          <tbody>
            ...
            <tr><td>终端</td><td>仅运行中容器显示。打开交互式 shell:<strong>默认 bash,可切换 sh</strong>(切换会重开会话);底部输入框输入命令回车执行,↑ / ↓ 翻本会话命令历史;输出保留最近 1000 行;<strong>终端尺寸随窗口自适应</strong>(自动同步远端行宽,长行按窗口宽度折行)。<strong>关闭终端按钮或关闭弹窗即断开会话</strong>;同一时间只允许一个终端会话。容器内操作与 SSH 登进该容器执行等效,删除类命令请谨慎</td></tr>
