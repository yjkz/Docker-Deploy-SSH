  ...
  var CHAPTERS = [
    ...
    },
    {
      ...
      html: `
        ...
        <ul>
          <li>设置中心「通用」→ <strong>部署日报时刻</strong>:填 0-23 表示每天该整点后发(留空 = 关闭);</li>
    ...
    },
    {
      ...
      html: `
        ...
        <ul>
          ...
          <li><strong>部署目录</strong> —— 服务器上的部署目录:可浏览 / 下载 / 查看文本;<strong>写权限按设置中心「通用」区的「宿主机可写目录」白名单</strong>——白名单内的目录可直接上传 / 编辑 / 新建 / 改名 / 删除,白名单外一律只读(改动请走部署与回滚流程,保持「归档即回滚点」的口径;留空 = 全部只读)。</li>
        </ul>
        <h4>操作与安全</h4>
        <ul>
