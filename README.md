...
- SSH 密码 / 私钥口令 / SMTP 密码在磁盘上**只以密文存在**——经 Windows DPAPI(CurrentUser 作用域)加密后存于 `config/`;WebView 里拿到的配置是「只读视图」(密文位置显示哨兵值 `*`),**真实密文不出后端**,前端也从不承载可解的密码。
- DPAPI 密文**绑定当前机器与当前用户**:直接拷贝 `config/` 目录到另一台机器(或换 Windows 用户)后,密文无法解开,**需要逐台重录密码**。
- 需要跨机器搬运配置时,用 03 页「配置中心」的**口令加密导出 / 导入**:导出文件由你的导出口令(Argon2id + AES-256-GCM)保护,导入到新机器时会在本机重新做 DPAPI 加密,密码即可继续使用。
