...
当前版本:v5.13.0(tauri.conf.json / Cargo.toml;第十四批:批量部署增强——失败/取消台的单台与一键批量续传(文档「批量不落断点」经核实为误,批量恒落断点)、「停止批量」可在步骤边界即时中止当前台、断点裁剪回收泄漏的临时 tar;顺带修复批量第二台永久卡死的 deferred 调用形态错配 bug)
```
