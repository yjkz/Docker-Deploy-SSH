...
- 主分支 `main`；当前版本 v6.16.0，`cargo test` 基线 **534 passed / 14 ignored**，命令 **122** 个（实测口径 = 数 `lib.rs` 的 `generate_handler![]`）。**版本与计数真值以 `ROADMAP.md`「当前状态速览」为准**（本行曾长期漂移）。
...
| `2026-09-13-待修复bug.md` | 历史审查清单（**已归档**：P0~P2 全修复，只读留档，不再维护） |
...
- 逻辑类改动先跑 `verify/` 脚本（比开浏览器快；清单见「常用命令」）；UI 视觉验证用**浏览器 + Tauri 桩**（`ui/_tauri-stub.js` + `ui/_judge-preview.html` + 本机无缓存静态服务，browser-use 截图交 judge）——**不用 computer-use**（用户常在全屏游戏）。**用完删桩、停服务、关页面**。
  - **IAB 按 URL 缓存资源**：桩验证前给预览页所有脚本**与 style.css** 加 `?v=N`（或起 no-cache 服务），否则改完 JS/CSS 看到的还是旧版本（第三十四批(四)实测：只破脚本缓存、未破样式表 → 版面类改动全部未生效却"渲染正常"）；`AppBus.on` 的 unlisten 是异步返回——任何「只判 unlisten 的幂等守卫」都无效（终端双注册事故，见 UPGRADE-PLAN 第二十二批二）。
