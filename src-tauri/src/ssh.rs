...
//! - [`SshClient::sftp_upload`] / [`SshClient::sftp_upload_dir`]:单文件与整目录上传,带字节进度回调
...
impl SshClient {
    ...
    pub async fn sftp_upload(
        ...
        let mut sent: u64 = 0;
        on_progress(0, total);
        copy_file_to_remote(&sftp, local, &remote_path, &mut sent, total, on_progress).await
    }
...
/// 把本地文件按 64KB 分块写入远端 SFTP 文件,累计 `sent` 并按块回调进度。
...
mod tests {
    ...
    async fn test_sftp_upload_real() {
        ...
        // 单文件上传到同一目录
        client
            .sftp_upload(&tmp.join("a.txt"), remote, "single.txt", &|_, _| {})
            .await
